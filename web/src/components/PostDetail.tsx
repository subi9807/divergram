...
<script async src="//www.instagram.com/embed.js"></script>`;
  ...
  const modalContent = (
    <div
      className={inline ? "w-full max-w-[980px] mx-auto py-4 px-2 md:px-4 min-h-[calc(100dvh-7rem)]" : "fixed inset-0 z-[9999] bg-black bg-opacity-75 flex items-center justify-center p-4"}
    ...
    >
      ...
      >
        <div className={inline ? "hidden md:flex flex-1 bg-black dark:bg-gray-950 items-center justify-center h-[70vh] min-h-[620px]" : "hidden md:flex flex-1 max-h-[90vh] bg-black dark:bg-gray-950 items-center justify-center"}>
          <MediaCarousel
            media={
        ...
        <div className={inline ? "w-full md:w-[420px] flex flex-col bg-white dark:bg-[#121212] md:max-h-none overflow-visible border-l border-gray-200 dark:border-[#262626]" : "w-full md:w-[420px] flex flex-col bg-white dark:bg-[#121212] max-h-[90vh] overflow-y-auto"}>
          ...
          <div className="border-b border-gray-300 dark:border-[#262626]">
            <div className="md:hidden w-full h-[300px] bg-black flex items-center justify-center overflow-hidden">
              {post.post_media && post.post_media.length > 0 ? (
                <MediaCarousel
                ...
                <div className="mt-3 rounded-2xl border border-sky-100 bg-sky-50/80 p-3.5 space-y-2.5 transition-colors dark:border-sky-900 dark:bg-sky-950/40">
                  {post.dive_type && (
                    <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400 font-semibold text-sm">
                      ...
                      {post.dive_type === 'scuba' ? '스쿠버다이빙' : post.dive_type === 'technical' ? '테크니컬다이빙' : '프리다이빙'}
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-2.5 text-xs">
                    ...
                    {post.gas_type && (
                      <div className="flex items-center gap-1 text-gray-700 dark:text-gray-300">
                        <Gauge className="h-3 w-3" />
                        <span>{post.gas_type}{post.gas_type !== 'air' && post.gas_percent ? ` ${post.gas_percent}%` : ''}</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="border-b border-gray-200 dark:border-[#262626] px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={toggleSave} className={`transition-colors ${isSaved ? 'text-yellow-400' : 'hover:text-gray-600 dark:text-white dark:hover:text-gray-300'}`}>
          ...
          <div className="px-5 py-4 space-y-4 dark:bg-[#121212]">            {comments.map((comment) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm dark:text-white">
                    <span className="font-semibold mr-2">{comment.profiles?.username || 'unknown'}</span>
