...
export default function MobileHeader({ currentPage, mobileBarsHidden, onNavigate, onToggleMenu }: MobileHeaderProps) {
  return (
    <header className={`dg-mobile-header fixed top-0 left-0 right-0 xl:hidden bg-white dark:bg-[#1b1d21] mx-auto mt-2 w-[calc(100%-14px)] rounded-[40px] shadow-[0_12px_30px_rgba(15,23,42,0.12)] border border-gray-300 dark:border-[#2a2e35] z-50 transition-transform duration-300 ease-out transition-colors ${mobileBarsHidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center space-x-8">
          <button onClick={() => onNavigate('home')} className="cursor-pointer" aria-label="Divergram 홈">
            <div className="w-10 h-10 text-white text-sm font-bold flex items-center justify-center">
              <img src="/assets/logo/divergram-logo-blue.png" alt="Divergram" className="mx-auto h-24 w-24 object-cover shadow-2xl shadow-sky-200/80 max-[40px]:h-10 max-[40px]:w-10 max-[760px]:h-10 max-[760px]:w-10" />
            </div>
          </button>
