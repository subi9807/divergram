...
export default function CreatePost({ onClose, onPostCreated }: CreatePostProps) {
      ...
      reader.onloadend = () => {
        ...
        if (newFiles.length === selectedFiles.length) {
          setFiles(prev => {
            ...
            if (next.some((file) => file.type === 'video')) setPublishToReels(true);
            return next;
          });
          setError('');
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!publishToFeed && !publishToReels) {
      setError('게시 위치를 하나 이상 선택해주세요.');
      return;
    }

    if (publishToReels && !files.some((file) => file.type === 'video')) {
      setError('릴스에는 영상 파일이 1개 이상 필요합니다.');
      return;
    }
    ...
    try {
      ...
      const { data: postData, error: insertError } = await db.from('posts').insert({
        ...
        publish_to_reels: publishToReels,
      }).select().single();
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-gray-50 dark:bg-[#0f0f0f] text-gray-900 dark:text-gray-100 dg-surface rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 z-20 flex items-center justify-between p-4 border-b border-gray-200 dark:border-[#262626] bg-white/95 dark:bg-[#121212]/95 backdrop-blur">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2 dark:text-white">
              <Waves className="h-6 w-6 text-blue-500" />
              새 다이빙 로그 작성
            </h2>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">미디어와 게시 위치만 필수예요. 나머지는 편하게 기록해도 됩니다.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-5">
          ...
          <section className="rounded-2xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/80 dark:bg-blue-950/20 p-4 shadow-sm">
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">
              게시 위치 *
            </label>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">피드와 릴스는 별도로 노출됩니다. 둘 다 선택할 수도 있어요.</p>
            <div className="flex gap-3 flex-wrap">
              <label className="flex items-center gap-2 rounded-lg bg-white dark:bg-[#1c1c1c] px-3 py-2 border border-gray-200 dark:border-gray-700 cursor-pointer">
                <input type="checkbox" checked={publishToFeed} onChange={(e) => setPublishToFeed(e.target.checked)} />
                <span className="text-sm font-medium">피드</span>
              </label>
              <label className="flex items-center gap-2 rounded-lg bg-white dark:bg-[#1c1c1c] px-3 py-2 border border-gray-200 dark:border-gray-700 cursor-pointer">
                <input type="checkbox" checked={publishToReels} onChange={(e) => setPublishToReels(e.target.checked)} />
          ...
          <section className="rounded-2xl border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#151515] p-4 shadow-sm">
            <div className="mb-3">
              <h3 className="text-base font-bold text-gray-900 dark:text-white">1. 미디어</h3>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">사진/영상을 최대 10개까지 올릴 수 있어요. 첫 번째 미디어가 대표로 보입니다.</p>
            </div>
          ...
          <section className="rounded-2xl border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#151515] p-4 shadow-sm">
            <div className="mb-4">
              <h3 className="text-base font-bold text-gray-900 dark:text-white">2. 다이빙 기본 정보</h3>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">다이빙 타입은 필수, 상세 수치는 선택 입력입니다.</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                다이빙 타입 *
          ...
          <section className="rounded-2xl border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#151515] p-4 shadow-sm space-y-4">
            <div>
              <h3 className="text-base font-bold text-gray-900 dark:text-white">3. 위치와 함께한 사람</h3>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">리조트, 지도 위치, 버디 태그를 추가하면 로그가 더 잘 정리돼요.</p>
            </div>
          ...
          <section className="rounded-2xl border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#151515] p-4 shadow-sm">
            <div className="mb-3">
              <h3 className="text-base font-bold text-gray-900 dark:text-white">4. 다이빙 컨디션</h3>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">느낀 점, 시야, 조류, 생물 등 자유롭게 남겨주세요.</p>
            </div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              다이빙컨디션
            </label>
          ...
          <div className="sticky bottom-0 -mx-4 sm:-mx-5 px-4 sm:px-5 py-3 bg-gray-50/95 dark:bg-[#0f0f0f]/95 backdrop-blur border-t border-gray-200 dark:border-[#262626] flex space-x-2">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              ...
              className="flex-1 px-3 py-2 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
