...
export type BottomNavItemId = 'home' | 'explore' | 'create' | 'resorts' | 'location' | 'reels' | 'messages' | 'notifications' | 'profile';

export const DEFAULT_BOTTOM_NAV_ITEMS: BottomNavItemId[] = ['home', 'explore', 'create', 'reels', 'profile'];
export const AVAILABLE_BOTTOM_NAV_ITEMS: BottomNavItemId[] = ['home', 'explore', 'create', 'resorts', 'location', 'reels', 'messages', 'notifications', 'profile'];

const normalizeBottomNavItems = (items: unknown): BottomNavItemId[] => {
  if (!Array.isArray(items)) return DEFAULT_BOTTOM_NAV_ITEMS;
  const valid = items.filter((item): item is BottomNavItemId => AVAILABLE_BOTTOM_NAV_ITEMS.includes(item as BottomNavItemId));
  const unique = Array.from(new Set(valid)).slice(0, 5);
  if (unique.length !== 5) return DEFAULT_BOTTOM_NAV_ITEMS;
  return unique;
};
...
type AppSettingsContextValue = {
  ...
  bottomNavItems: BottomNavItemId[];
  setBottomNavItems: (items: BottomNavItemId[]) => void;
  resetBottomNavItems: () => void;
  t: (key: string) => string;
};
...
export function AppSettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [bottomNavItems, setBottomNavItemsState] = useState<BottomNavItemId[]>(() => {
    try { return normalizeBottomNavItems(JSON.parse(localStorage.getItem('dg_bottom_nav_items') || '[]')); } catch { return DEFAULT_BOTTOM_NAV_ITEMS; }
  });
  ...
  useEffect(() => { localStorage.setItem('dg_bottom_nav_items', JSON.stringify(bottomNavItems)); }, [bottomNavItems]);

  const setBottomNavItems = (items: BottomNavItemId[]) => {
    setBottomNavItemsState(normalizeBottomNavItems(items));
  };

  const resetBottomNavItems = () => {
    setBottomNavItemsState(DEFAULT_BOTTOM_NAV_ITEMS);
  };
  ...
  }), [country, language, units, privacyScope, theme, wearables, bottomNavItems]);
