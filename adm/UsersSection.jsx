...
export default function UsersSection({
  ...
  useEffect(() => {
    const allowed = new Set((visibleUsers || []).map((user) => String(user?.id || '').trim()).filter(Boolean));
  ...
  const allSelected = visibleUsers.length > 0 && visibleUsers.every((user) => selectedIds.includes(String(user.id)));
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedIds((prev) => {
      if (allSelected) return prev.filter((id) => !visibleUsers.some((user) => String(user.id) === String(id)));
      return Array.from(new Set([...prev, ...visibleUsers.map((user) => String(user.id)).filter(Boolean)]));
    });
  };
  ...
  return (
    <div className="card users-card-wrap">
      ...
      <div className="table-toolbar">
        ...
        <span className="table-selection-count">선택 {selectedCount}명</span>
        <div className="table-toolbar-actions">
          <button type="button" className="sm danger" onClick={bulkDeleteSelected} disabled={!selectedCount}>선택 회원 삭제{selectedCount ? ` (${selectedCount})` : ''}</button>
        </div>
      </div>
                  ...
                  <td className="table-select-cell" onClick={(e) => e.stopPropagation()}>
                    <label className="table-check">
                      <input
                        ...
                        aria-label={`회원 ${user.username || user.id} 선택`}
                      />
                    </label>
                  </td>
                  <td>
