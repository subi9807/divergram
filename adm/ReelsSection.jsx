...
function ReelRow({ post, onDelete, getReelThumb, checked, onToggleSelect }) {
...
export default function ReelsSection({ reelRows, reelPageRows, reelPage, reelTotalPages, setReelPage, getReelThumb, onDelete, onBulkDelete }) {
  const [selectedIds, setSelectedIds] = useState([]);

  useEffect(() => {
    const allowed = new Set((reelRows || []).map((item) => String(item?.id || '').trim()).filter(Boolean));
    setSelectedIds((prev) => prev.filter((id) => allowed.has(String(id))));
  }, [reelRows]);

  const pageIds = useMemo(() => (reelPageRows || []).map((item) => String(item?.id || '').trim()).filter(Boolean), [reelPageRows]);
  ...
  return (
    <div className="card admin-section">
      ...
      <table>
        ...
        <tbody>{reelPageRows.map((post) => <ReelRow key={post.id} post={post} getReelThumb={getReelThumb} checked={selectedIds.includes(String(post.id))} onToggleSelect={toggleSelect} onDelete={onDelete} />)}</tbody>
      </table>
