export function registerAdminDataRoutes(app, { pool, requireAdmin, bcrypt, crypto }) {
  app.post('/api/admin/migrate-normalized', requireAdmin, async (_req, res) => {
    try {
      const mapping = [
        ['profiles', 'app_profiles', ['id','username','full_name','bio','avatar_url','website','account_type','scuba_level','freediving_level','license_type','license_number','license_agency','license_issued_at','license_image_url','created_at']],
  ...
  app.get('/api/admin/tables', requireAdmin, async (_req, res) => {
    const tableNames = ['app_users','app_profiles','app_posts','app_post_media','app_likes','app_comments','app_follows','app_saved_posts','app_notifications','app_rooms','app_participants','app_messages','app_reports','app_moderation_actions','app_ad_slots','admin_audit_logs'];
    try {
      const out = [];
  ...
  app.get('/api/admin/table/:name', requireAdmin, async (req, res) => {
    ...
    const allow = new Set(['app_users','app_profiles','app_posts','app_post_media','app_likes','app_comments','app_follows','app_saved_posts','app_notifications','app_rooms','app_participants','app_messages','app_reports','app_moderation_actions','app_ad_slots','admin_audit_logs']);
