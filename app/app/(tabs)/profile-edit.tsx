...
export default function ProfileEditScreen() {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      await apiClient.updateProfile(user.id, nextProfilePatch);
          ...
          <View className="mb-5 items-center">
            ...
            <View className="mt-3 flex-row">
              <TouchableOpacity
                ...
                activeOpacity={0.88}
                onPress={pickAvatar}
                disabled={pickerBusy || avatarUploading}
              >
                {avatarUploading ? <ActivityIndicator size="small" color="#0f172a" /> : <ImagePlus size={15} color="#0f172a" />}
                <Text className="ml-1 text-xs font-semibold text-gray-900 dark:text-surface-50">
                  {avatarUploading
                    ? t('common.loading', { defaultValue: '업로드 중...' })
