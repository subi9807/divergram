...
export default function MessagesScreen() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: () => apiClient.getMessageRooms(String(user?.id || '')),
  });
    ...
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 28 }}>
        ...
        <View className="px-5 py-1">
          {data.length === 0 ? (
            <View className="items-center rounded-3xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-8">
              <MessageCircle size={30} color={isDark ? '#7dd3fc' : '#0d5fa8'} />
              <Text className="mt-3 text-sm text-surface-500 dark:text-surface-400">{t('common.noData', { defaultValue: '메시지가 없습니다.' })}</Text>
            </View>
          ) : null}
          {data.map((room) => (
            <TouchableOpacity
