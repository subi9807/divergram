...
export default function SettingsDetailScreen() {
  ...
  }, [socialProvider, syncSocialLinks]);
  ...
  const handleLinkProvider = async () => {
    ...
    try {
      ...
      const socialLink = error?.socialLink;
      const code = String(error?.code || error?.message || error?.response?.data?.error || '').trim();
      if (socialLink && code.includes('sso_email_exists')) {
        try {
          await linkSocialAccount(socialLink, { requireCurrentSession: true });
          await refreshSocialLinks();
          showToast({
            type: 'success',
            title: t('common.success', { defaultValue: '완료' }),
            message: `${socialLabel} ${t('settingsPage.account.linkAction', { defaultValue: '연동하기' })} ${t('common.done', { defaultValue: '완료' })}`,
          });
          return;
        } catch (linkError: any) {
          const linkMessage = String(linkError?.response?.data?.error || linkError?.message || '').trim();
          showToast({
            type: 'error',
            title: t('auth.error', { defaultValue: '오류' }),
            message: linkMessage.includes('oauth_email_mismatch')
              ? t('auth.socialLinkEmailMismatch', { defaultValue: '현재 로그인한 계정 이메일과 소셜 계정 이메일이 달라 연동할 수 없습니다.' })
              : t('auth.socialLinkFailed', { defaultValue: '소셜 계정 연동에 실패했습니다. 다시 시도해주세요.' }),
          });
          return;
        }
      }
      ...
      [
        { text: t('common.cancel', { defaultValue: '취소' }), style: 'cancel' },
        {
          ...
          onPress: async () => {
            setBusy(true);
            try {
              await apiClient.unlinkOAuthLink(socialProvider);
              await refreshSocialLinks();
              showToast({
                type: 'info',
                title: t('common.done', { defaultValue: '완료' }),
                message: `${socialLabel} ${t('settingsPage.account.unlinkAction', { defaultValue: '연동끊기' })}`,
              });
            } catch {
          ...
          {socialProvider ? (
            <View className="rounded-3xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4">
              ...
              <View className="flex-row">
                <TouchableOpacity
                  activeOpacity={0.86}
                  disabled={busy || socialLinked}
                  onPress={handleLinkProvider}
                  className={`mr-2 flex-1 rounded-xl px-3 py-3 ${busy || socialLinked ? 'bg-surface-300 dark:bg-surface-700' : 'bg-brand-600'}`}
                >
                  <Text className={`text-center text-sm font-semibold ${socialLinked ? 'text-surface-600 dark:text-surface-300' : 'text-white'}`}>
                    {busy
                      ? t('common.loading', { defaultValue: '처리중...' })
                      : socialLinked
                        ? t('common.done', { defaultValue: '완료' })
                        : t('settingsPage.account.linkAction', { defaultValue: '연동하기' })}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  activeOpacity={0.86}
                ...
                >
                  <Text className={`text-center text-sm font-semibold ${socialLinked ? 'text-red-600' : 'text-surface-400 dark:text-surface-500'}`}>
                    {t('settingsPage.account.unlinkAction', { defaultValue: '연동끊기' })}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
