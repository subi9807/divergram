          ...
          >
            <View style={{ borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#fff' }}>
              ...
              <View style={{ padding: 8 }}>
                <Text numberOfLines={1} style={{ fontSize: 12, fontWeight: '700', color: '#0F172A' }}>{item.title}</Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 3 }}>
                  <MapPin size={11} color="#64748B" />
                  <Text numberOfLines={1} style={{ marginLeft: 3, fontSize: 11, color: '#64748B', flex: 1 }}>{item.location}</Text>
                </View>
              </View>
            </View>
          </TouchableOpacity>
        ...
        ListHeaderComponent={
          <>
            <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12 }}>
              <Text style={{ fontSize: 28, fontWeight: '800', color: '#0F172A' }}>{t('explore.title')}</Text>
              <Text style={{ marginTop: 4, color: '#64748B' }}>{t('explore.subtitle')}</Text>
              <View style={{ marginTop: 14, height: 48, flexDirection: 'row', alignItems: 'center', borderRadius: 16, borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#fff', paddingHorizontal: 14 }}>
                <Search size={18} color="#64748B" />
                <TextInput
                  value={searchText}
                  onChangeText={setSearchText}
                  style={{ marginLeft: 8, flex: 1, color: '#0F172A' }}
                ...
                renderItem={({ item, index }) => (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderColor: '#E2E8F0',
                      backgroundColor: activeTopicIndex === index ? '#0D5FA8' : '#fff',
                    }}
                  >
                    <Text style={{ color: activeTopicIndex === index ? '#fff' : '#334155', fontWeight: '700' }}>{item}</Text>
                  </TouchableOpacity>
                )}
              />
            </View>
