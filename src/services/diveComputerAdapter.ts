...
import { buildMockExternalLogs } from './externalDiveLogMapper';
import { mockDiveDevices } from '../mock/divergramExpansionMock';
...
export class GenericBluetoothAdapter implements DiveComputerAdapter {
  ...
  async scanDevices(): Promise<DiveDevice[]> {
    ...
    return mockDiveDevices
      .filter((row) => !row.brand || row.brand === 'other')
      .map((row) => ({
        ...row,
        rssi: withRssiJitter(row.rssi ?? -70),
        lastSeenAt: nowIso(),
...
abstract class BrandedBluetoothAdapter extends GenericBluetoothAdapter {
  ...
  constructor(provider: 'garmin' | 'suunto' | 'shearwater', fallbackDevice: DiveDevice, selectedDevice?: DiveDevice | null) {
    super(selectedDevice || fallbackDevice);
  ...
  protected get currentDevice(): DiveDevice {
    return (
      this.deviceTemplate || {
        id: `dev-${this.provider}`,
        name: `${this.provider} dive computer`,
        brand: this.provider,
        isConnectable: true,
        protocol: 'ble',
      }
...
export class GarminBluetoothAdapter extends BrandedBluetoothAdapter {
  ...
  async getDiveLogList(): Promise<ExternalDiveLog[]> {
    const all = await fetchGarminDiveLogs().catch(() => buildMockExternalLogs('garmin'));
...
export class SuuntoBluetoothAdapter extends BrandedBluetoothAdapter {
  ...
  async getDiveLogList(): Promise<ExternalDiveLog[]> {
    const all = await fetchSuuntoDiveLogs().catch(() => buildMockExternalLogs('suunto'));
...
export class ShearwaterBluetoothAdapter extends BrandedBluetoothAdapter {
  ...
  async getDiveLogList(): Promise<ExternalDiveLog[]> {
    const all = await fetchShearwaterDiveLogs().catch(() => buildMockExternalLogs('shearwater'));
